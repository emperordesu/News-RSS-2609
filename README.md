...
히어로(오늘의 요약) 바로 아래에 현대차(005380)·기아(000270)·현대모비스(012330) 시세를 애플 스타일의 컴팩트한 카드로 보여줍니다. 네이버 증권은 종목별 RSS를 제공하지 않아 시세 페이지의 스크린리더용 요약 블록(`<dl class="blind">…</dl>`)을 정규식으로 파싱합니다(`src/stocks.js`). 사이트 구조 변경 등으로 파싱에 실패하면 해당 종목은 `data/stocks.json`에 저장된 마지막 성공 값을 그대로 유지하고, 에러만 콘솔에 로그로 남깁니다. 장 마감 이후에는 카드 하단에 "전일 종가 기준" 라벨이 표시됩니다. 종목을 추가/변경하려면 `src/stocks.js`의 `STOCKS` 배열을 수정하세요.

## 기간 필터 (24시간 / 7일 / 30일)

상단 내비게이션의 세그먼트 버튼은 서버 재요청 없이 클라이언트 사이드 JavaScript로 동작합니다(`src/site.js` 하단에 인라인 삽입). 각 기사 카드의 `data-pubdate` 속성을 기준으로 카드/섹션/국가별 바로가기를 보이거나 숨기고, 히어로의 헤드라인·요약 문장도 선택한 기간에 맞춰 다시 계산합니다(서버 쪽 `src/briefing.js`의 키워드 추출 로직을 JS로 그대로 이식). 기본값은 "24시간"이며, 30일 필터가 의미 있으려면 `RETENTION_DAYS`가 최소 30이어야 합니다(현재 30으로 설정됨).
...
로고 아래, 오늘의 요약 위에 최신 기사 상위 6건을 자동으로 넘어가는 풀블리드 캐러셀이 표시됩니다(`renderHeroCarousel`, kia.com/jp/ja 참고). 5초 간격 자동 전환, 좌우 화살표, 1~6 번호가 붙은 하단 진행바 인디케이터로 직접 이동할 수 있고, 마우스를 올리면 일시정지됩니다. 썸네일 이미지가 없거나 로드에 실패하면 해당 매체의 색상+이니셜이 자동으로 대체 표시됩니다.
